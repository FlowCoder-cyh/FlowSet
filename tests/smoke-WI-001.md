...
| WI-001-3 | `init.md` Step 3.5 case 블록 존재 + 3종 class validation (WI-B1 단일 case도 수용) + 미지정 값 exit 1 | 3 |
...
| WI-001-6 | 학습 전이 회귀 방지 (패턴 2 `((var++))` / 패턴 4 `\|\| echo 0` / 패턴 19 `local x=$(cmd)`, 백틱·주석 anti-example 제외) | 3 |
| **합계** | | **40** |
