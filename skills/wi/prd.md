...
설계 §4 :98-107이 정의한 5종에 한정합니다. Python 기반 auth(`flask-login`, `django.contrib.auth` 등)는 v4.0 범위 외이며, 후속 WI 또는 Step 2.5.c 커스텀 수동 추가 경로로 처리합니다.
...
generate_code_matrix() {
  # 입력 1 (인자): class — "code" | "hybrid" (기본 "code")
  #   - hybrid 분기에서 호출 시 "hybrid"로 전달 → 결과 matrix.json의 class 필드를 정확히 set.
  #   - generate_hybrid_merge_content가 후속으로 sections를 병합하기 전, entities 영역을 먼저 만든다.
  # 입력 2 (파일): prd-state.json의 entities[], roles[], crud_matrix{}, permission_matrix{}, auth_patterns[], auth_framework
  # 출력: .flowset/spec/matrix.json (class=$1 스키마)
  local class_arg="${1:-code}"
...
생성 흐름은 2단계입니다.
1. `generate_code_matrix hybrid` 호출 — entities + class="hybrid" 채움 (auth_framework / auth_patterns도 포함).
2. `generate_hybrid_merge_content` 호출 — 위에서 생성한 matrix.json에 sections 병합 (class 필드는 그대로 hybrid 유지).

```bash
generate_hybrid_merge_content() {
  # 사전 조건: generate_code_matrix hybrid가 .flowset/spec/matrix.json을 이미 생성한 상태.
  # 책임: sections 병합 (entities + class은 보존, sections만 추가).
  # 입력: prd-state.json의 sections[], roles[]
  local matrix=".flowset/spec/matrix.json"
  local roles_json sections_json
  roles_json=$(jq '.roles' .flowset/prd-state.json)
  sections_json=$(jq '.sections // []' .flowset/prd-state.json)

  # entities + class은 보존, sections만 신규 set
  jq --argjson roles "$roles_json" \
     --argjson sections "$sections_json" '
    .sections = (
      ($sections | map({
...
**Step 4.M.d: matrix.json 생성 진입점 + 셀 의무 검증**

`/wi:prd` Step 4 마지막에 PROJECT_CLASS에 따라 분기 후 즉시 검증:

```bash
mkdir -p .flowset/spec
case "${PROJECT_CLASS:-code}" in
  code)    generate_code_matrix code ;;
  content) generate_content_matrix ;;
  hybrid)  generate_code_matrix hybrid; generate_hybrid_merge_content ;;
  *)       echo "ERROR: 알 수 없는 PROJECT_CLASS: ${PROJECT_CLASS}" >&2; exit 1 ;;
esac

# 셀 의무 self-check — 미완성 셀이 있으면 즉시 종료
# 후속 단계(WI 생성, fix_plan.md 작성)로 미완성 매트릭스가 흘러가는 것을 차단.
verify_matrix_cells || exit 1
```
    ...
    hybrid)
      ...
      missing=$(jq -r '.entities | to_entries[] |
        select((.value.crud | keys | sort) != ["C","D","R","U"] or
               (.value.status | keys | sort) != ["C","D","R","U"]) |
        .key' "$matrix")
      [[ -n "$missing" ]] && { echo "ERROR: hybrid CRUD 셀 누락 entity: $missing" >&2; return 1; }
      missing=$(jq -r '.sections | to_entries[] |
        select((.value.status | keys | sort) != ["approve","draft","review"]) |
        .key' "$matrix")
      [[ -n "$missing" ]] && { echo "ERROR: hybrid Section status 셀 누락: $missing" >&2; return 1; }
      ;;
    *)
      echo "ERROR: matrix.json class 값 비정상: $class" >&2
      return 1
      ;;
  esac
  return 0
...
이 함수는 위 case 진입점 마지막의 `verify_matrix_cells || exit 1`로 강제 호출됩니다. prose 가 아닌 실 호출이므로, hybrid에서 entities는 OK인데 sections만 누락된 경우도 즉시 차단됩니다.
