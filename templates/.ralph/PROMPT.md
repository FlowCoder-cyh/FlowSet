...
- `.ralph/AGENT.md` 읽기 → 빌드/테스트 명령 + **"인프라 환경"** (DB, mock 금지) + **"와이어프레임"** 경로 확인
- `.ralph/guardrails.md` 읽기 → 프로젝트별 금지사항 확인
- `.ralph/contracts/` 존재 시 → api-standard.md, data-flow.md 읽기 (API/데이터 작업 시 필수)
...
# merge queue에 등록 (머지 대기는 ralph.sh가 관리)
bash .ralph/scripts/enqueue-pr.sh $(gh pr view --json number --jq '.number')
```

**⚠️ CI 폴링/머지 대기/main 복귀는 하지 않음** — 외부 루프(ralph.sh)가 처리합니다.
워커는 enqueue 후 즉시 RALPH_STATUS를 출력하고 종료하세요.
