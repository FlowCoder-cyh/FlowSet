...
# 브랜치 네이밍: {category}/WI-{ID}-{type}-{작업명-kebab}
# ID = fix_plan.md에서 해당 WI의 영숫자 ID — [0-9A-Za-z]+(-[0-9]+)? (예: 001, A2a, C3code, E1, 001-1)
# feat → feature/, fix → fix/, chore → chore/, docs → docs/, refactor → refactor/
# kebab 변환: 한글→영문 요약, 공백→하이픈, 소문자, ASCII만
# 예: "사용자 인증 추가" → "user-auth", WI ID = 001 또는 A2a
...
git add {변경된 파일들}
git commit -m "WI-{ID}-{type} {한글 작업명}"  # 반드시 한글 포함 (ID는 fix_plan 영숫자 — 예: 001, A2a, C3code, E1)
